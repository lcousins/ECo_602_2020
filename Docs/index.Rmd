...
*\*Note that the answers to the questions are at the vey bottom of the page.*
...
## $
The dollar sign can be used if you know the name of a column, but not the index.  It is useful if other people (who may not be as familiar with you as the data) will be reading your code.  Also, it is useful because you may not always remember what numbers correspond to what columns.
```{r, fig.height=7, class.output="scroll-250"}
...
```{r, fig.height=7, class.output="scroll-250"}
subset(penguins,  bill_length_mm < 190)
```
...
# I can print the contents of the matrix by simply typing the variable name
matrix_1
matrix_1a

# If I want to subset the matrix, maybe by only printing the second row, I would use the following code
matrix_1[2, ]
...
- minimum, 1st quartile, median, mean, 3rd quartile, maximum and the number of NA values for numerical values
...
- If you don't use the $ sign, you must use the `data =` argument, which is simply the name of the dataset you want to plot.  
...
The parameter `mfrow=` can be used if you want to display  multiple plots on one display. It is useful if you need ot make side-by-side comparisons, especially if there is at least one shared variable between the graphs.
```{r, fig.height=7, class.output="scroll-250"}
...
##`library()`
The library function loads data packages. It is how you can get data into the working directory.  If the package has not been installed beforehand, however, it will not execute. If you  are confused about the difference between library and require see this [site](https://yihui.org/en/2014/07/library-vs-require/).
```{r, fig.height=7, class.output="scroll-250"}
...
##`read.csv()`
The `read.csv()` function is another way to read data into the working directory.  You can use a file path (relative to the working directory).
```{r, fig.height=7, class.output="scroll-250"}
...
```{r, fig.height=7, class.output="scroll-250"}
histogram_2 <- hist(penguins$body_mass_g, 
  col=16, 
  xlab='Body Mass', 
  main='Histograms')
...
```{r, fig.height=7, class.output="scroll-250"}
histogram_3 <- hist(penguins$bill_length_mm,
  col=16, 
  xlab='Bill Length', 
  main='Histograms')
```
...
```{r, fig.height=7, class.output="scroll-250"}
boxplot_2 <- boxplot(body_mass_g~species,
        data = penguins,
        ylab = 'Body Mass',
        xlab = 'Species',
        main = 'Boxplots', 
...
```{r, fig.height=7, class.output="scroll-250"}
boxplot<- boxplot(body_mass_g~sex,
        data = penguins,
        ylab = 'Body Mass',
        xlab = 'Sex',
        main = 'Boxplots',
...
boxplot(body_mass_g ~ sex + species, 
        ...
        names = list('1','2','3','4','5','6'),
        col=16)
# create a legend for the boxplot
legend("topleft", inset =  .03, legend= c('1 - F Adelie', '2 - M Adelie', '3 - F Chinstrap', '4 - M Chinstrap', '5 - F Gentoo', '6 - M Gentoo'), box.lty = 1)
```
...
When not accounting for species differentiation, the relationship between body mass and length seems mostly linear with a positive slope. It could also be interprested as slightly concave down, increasing.  There are a few points with a high length, and these likely correspond to one specific species.

#### Qualitatively describe the shapes of the histograms. Do the data appear normally-distributed? Explain why or why not.
The histogram of body mass appears to be skewed right slightly. The mode is aroung 3500, which is sharply decreasing to the left side.  However, looking at the right side, the frequency diminishes more gradually.

The histogram of flipper length appears to be bimodal when graphed with 14 bins.  Even when graphed with 7 bins, the data appears bimodal, however it is less obvious in this case. 
...
Since the p-value for the shapiro test on the body length is so low, I can conclude that the data is *not* normally distributed.  The shapiro test on the body mass is also quite low, so I can conclude that that is also *not* normally distributed too.  From a visual inspection, I can see how this is the case,  since both histograms have elements that make me doubt the normality..  
...
The males are heavier than the females for all species.  Furthermore, irrespective of gender, the Gentoo penguins appear to be heavier than the others. In general, the Adelie appear to be slightly heavier than the Adelie, however there is some overlap becaise the weights are so similar.
...
Based on the results of the residual test, I can conclude that none of the residual models are normally distributed.  From the shapiro test, I can see that the p-value is quite low, suggesting that I can reject the null hypothesis that they would be normally distributed, and instead conclude that they are not.  From a rough visual inspection, this is less obvious, however, when inspecting more closely, I can see how it is true.  
...
The expected body length of an animal that weighs 100g is 163.67466 mm.
...
The two models that have the lowest AIC are the additive model and  the single-predictor model of species.
...
I would use the additive model because, even though it is more complex to write/understand, it simplifies the overall process by only requiring one function.  Also, the model fits the data quite well, and it has low p-values for both species and sex.
